...
def upload_and_execute_script(host, username, password, local_script_path, remote_script_name):
    ...
    try:
        ...
        ssh_shell.send(f"/home/{username}/{remote_script_name}\n")
        time.sleep(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upload and execute script via SSH")
    parser.add_argument("--host", required=True, help="Target host IP address")
    parser.add_argument("--username", required=True, help="SSH username")
    parser.add_argument("--password", required=True, help="SSH password")
    parser.add_argument("--local-script", required=True, help="Local path to the script")
    parser.add_argument("--remote-script-name", required=True, help="Name of the remote script")
